...
const CalculateScreen: React.FC = () => {
	...
	const handleDecrement = (id: string, calories: number) => {
		setServings((prevServings) => {
			const newServings = {
				...prevServings,
				[id]: Math.max((prevServings[id] || 0) - 1, 0),
			};
			calculateTotalCalories(newServings);
			calculateTotalProtein(newServings); //2
			calculateTotalCarbs(newServings); //3
	...
	const calculateTotalCalories = (newServings: {[id: string]: number}) => {
		// Key-value pair of food ID and its number of servings
		let total = 0;
		data.forEach((item) => {
	...
	const calculateTotalProtein = (newServings: {[id: string]: number}) => {
		// Key-value pair of food ID and its number of servings
		let total = 0;
		data.forEach((item) => {
	...
	const calculateTotalCarbs = (newServings: {[id: string]: number}) => {
		// Key-value pair of food ID and its number of servings
		let total = 0;
		data.forEach((item) => {
	...
	const calculateTotalFat = (newServings: {[id: string]: number}) => {
		// Key-value pair of food ID and its number of servings
		let total = 0;
		data.forEach((item) => {
			...
			<View style={styles.container}>
				...
				<ScrollView style={styles.scrollBox}>
					{error ? (
						...
						data.map(
							(
								item, // KEEP THIS VVVV HANDLEFOODPRESS AS TEXT AND NOT PRESSABLE BC FLEX AND WRAP GETS MESSED UP
							) => (
								<View key={item.id} style={styles.itemContainer}>
									...
									<View style={styles.buttonContainer}>
										<TouchableOpacity
											onPress={() => handleDecrement(item.id, item.calories)}
											style={styles.decrementButton}
										>
											<Text style={styles.buttonText}>-</Text>
										</TouchableOpacity>

										<Text style={[styles.servingText, dynamicStyles.text]}>
											{servings[item.id] || 0}
										</Text>
										<TouchableOpacity
											onPress={() => handleIncrement(item.id, item.calories)}
									...
									<View style={styles.nutritionLabelContainer}>
										...
										<View style={styles.row}>
											...
											<Text style={styles.valueText3}>
												{selectedFood.total_fat}g
											</Text>
											<Text style={styles.percentText}>
												{selectedFood.total_fat_percent.toFixed(0)}%
											</Text>
										</View>
										...
										<View style={styles.row}>
											...
											<Text style={styles.valueText3}>
												{selectedFood.saturated_fat}g
											</Text>
											<Text style={styles.percentText}>
												{selectedFood.saturated_fat_percent.toFixed(0)}%
											</Text>
										</View>
										<View style={styles.horizontalLine5} />
										<View style={styles.row}>
										...
										<View style={styles.row}>
											...
											<Text style={styles.percentText}>
												{selectedFood.sodium_percent.toFixed(0)}%
											</Text>
										</View>
										<View style={styles.horizontalLine5} />
										<View style={styles.row}>
											...
											<Text style={styles.valueText3}>
												{selectedFood.total_carbohydrates}g
											</Text>
											<Text style={styles.percentText}>
												{selectedFood.total_carbohydrates_percent.toFixed(0)}%
											</Text>
										</View>
										<View style={styles.horizontalLine5} />
										<View style={styles.row}>
											...
											<Text style={styles.valueText3}>
												{selectedFood.dietary_fiber}g
											</Text>
											<Text style={styles.percentText}>
												{selectedFood.dietary_fiber_percent.toFixed(0)}%
											</Text>
										</View>
										<View style={styles.horizontalLine5} />
										<View style={styles.row}>
										...
										<View style={styles.row}>
											...
											<Text style={styles.valueText4}>
												Includes {selectedFood.added_sugars}g Added Sugars
											</Text>
											<Text style={styles.percentText}>
												{selectedFood.added_sugars_percent.toFixed(0)}%
											</Text>
										</View>
										<View style={styles.horizontalLine5} />
										<View style={styles.row}>
...
function useStyles() {
	...
	return StyleSheet.create({
		container: {
			flex: 1,
			padding: 1,
			backgroundColor: mainTheme.colors.background, // BACKGROUND FOR CALCULATE PAGE. COLOR
		},
		mainContent: {
			padding: 50,
		...
		servingText: {
			//Incrementer value between - and + containers
			fontSize: 18,
